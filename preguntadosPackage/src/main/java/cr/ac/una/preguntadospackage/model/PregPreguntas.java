...
@NamedQueries({
    ...
    @NamedQuery(name = "PregPreguntas.findByParameters", query = "SELECT p FROM PregPreguntas p upper(p.id) LIKE :id AND upper(p.textoPregunta) LIKE :textoPregunta AND upper(p.cantidadRespuestas) LIKE :cantidadRespuestas AND upper(p.cantidadAciertos) LIKE :cantidadAciertos AND upper(p.estado) LIKE :estado"),
